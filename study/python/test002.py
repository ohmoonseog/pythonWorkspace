    ...
    with io.open(data_name , "a" , encoding="utf-8") as outfile:
        with io.open(log_file, 'r',encoding="utf-8") as infile:
            with apache.ApacheSource(infile) as source:
                for row in source:
                    if ( row.status != 408 and row.status != 404  ):
                        file = ""
                        ext = ""
...
if __name__ == '__main__':
    ...
    idx = 0
    for log_gbn in log_data.get('file_gbn'):
        log_path = log_data.get('file_path').get(idx)
        idx = idx + 1
        if ( os.path.isdir(log_path) ) == True:
            ...
            if accessJob == "Y" :
                ...
                if proc_cnt > 0 :
                    pool = Pool(proc_cnt)
                    pool.map(fn_file_create,log_fileList)
                    pool.close()
                    pool.join()
